# -*- coding: utf-8 -*-
"""
Для baseline оценки генеративных текстовых моделей: автоэнкодер и вариационный
автоэнкодер предложений на базе LSTM+Conv (точная конфигурация нейросети настраивается
переменной NET_CONFIG).

Модель тренируется на подготовленном заранее датасете - см. скрипты prepare_phrases.py
и prepare_vae_dataset.py

Натренированную модель можно загрузить и сгенерировать несколько предложений, подавая
на вход декодера случаный вектор вместо вектора скрытых переменных предложения.
"""
...
if do_train:
    ...
    nb_val_batches = min(200, val_data.shape[0]//BATCH_SIZE)
